class Route
  ...
  def self.all
    @@all_routes.each_index do |x|
      puts "Индекс #{x} маршрут #{@@all_routes[x]}"
    end
  end
  ...
  def validate!
    msg = 'Маршрут не создан. В маршруте должно содержаться как минимум' +
          + ' 2 станции'
    raise msg if @stations.size < 2
  end
