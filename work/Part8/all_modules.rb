module Menu
  ...
  def get_menu_station
    menu_generator("Меню станции", ["выйти из меню", "создать станцию", "посмотреть все станции", "посмотреть все поезда на станции", "посмотреть поезд определенного типа", "Информация о станциях"])
  end

  def get_menu_train
    menu_generator("Меню поезда",  ["выйти из меню", "создать поезд", "добавить вагон к поезду", "отцепить вагон от поезда", "подвинуть поезд по маршруту", "найти поезд", "добавить поезду маршрут", "занять место/объем в вагоне поезда"])
  end
